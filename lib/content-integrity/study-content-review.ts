...
function formatContext(item: any, context: Awaited<ReturnType<typeof fetchBlueprintContext>>) {
  return {
    ...
    applied_knowledge_statement: context.objective?.title || item.applied_knowledge_statement || item.learning_objective || 'Not provided',
    ...
    learning_objective: context.objective?.learning_objective || context.subtopic?.learning_objective || item.learning_objective || item.applied_knowledge_statement || 'Not provided',
    ...
    cacrep_core_areas: Array.isArray(item.cacrep_core_areas) ? item.cacrep_core_areas : [],
    blueprint_version: item.blueprint_version || 'stored-blueprint',
